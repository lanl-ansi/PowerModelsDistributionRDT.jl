...
@testset "test data input" begin
    ...
    @testset "test ref input" begin
        data = parse_file(example_data)
        pm = _PMD.instantiate_mc_model(data, _PMD.ACPUPowerModel, build_mc_rdt; ref_extensions=[ref_add_rdt!], eng2math_extensions=[transform_switch_inline_ne!, transform_switch_inline!], multinetwork=true)
