...
function constraint_ue(pm::AbstractUnbalancedPowerModel, i::Int; nw::Int=nw_id_default, base_nw::Int=nw_id_default)
    constraint_ue(pm, nw, base_nw, i)
end

function constraint_xe(pm::AbstractUnbalancedPowerModel, i::Int; nw::Int=nw_id_default, base_nw::Int=nw_id_default)
    constraint_xe(pm, nw, base_nw, i)
end

function constraint_te(pm::AbstractUnbalancedPowerModel, i::Int; nw::Int=nw_id_default, base_nw::Int=nw_id_default)
    constraint_te(pm, nw, base_nw, i)
end

function constraint_he(pm::AbstractUnbalancedPowerModel, i::Int; nw::Int=nw_id_default, base_nw::Int=nw_id_default)
    constraint_he(pm, nw, base_nw, i)
end


function constraint_critical_load(pm::AbstractUnbalancedPowerModel; nw::Int=nw_id_default)
    limit = pm.ref[:it][pmd_it_sym][:critical_load_met]
    conductors = ref(pm, nw, :conductors)
...
function constraint_total_load(pm::AbstractUnbalancedPowerModel; nw::Int=nw_id_default)
    limit = pm.ref[:it][pmd_it_sym][:total_load_met]
    conductors = ref(pm, nw, :conductors)
...
function constraint_mc_power_balance_shed_ne(pm::AbstractUnbalancedPowerModel, i::Int; nw::Int=nw_id_default)::Nothing
    ...
    bus_arcs_ne = ref(pm, nw, :bus_arcs_conns_branch_ne, i)
    bus_arcs_sw = ref(pm, nw, :bus_arcs_conns_switch, i)
    bus_arcs_sw_ne = ref(pm, nw, :bus_arcs_conns_switch_inline_ne, i)
    bus_arcs_trans = ref(pm, nw, :bus_arcs_conns_transformer, i)
    bus_arcs_trans_ne = ref(pm, nw, :bus_arcs_conns_transformer_ne, i)
...
"""
function constraint_mc_thermal_limit_from_damaged(pm::AbstractUnbalancedPowerModel, i::Int; nw::Int=nw_id_default)::Nothing
    branch = ref(pm, nw, :branch, i)
    f_idx = (i, branch["f_bus"], branch["t_bus"])
    bus = ref(pm, nw, :bus, branch["f_bus"])
    ...
    rating = calc_branch_power_max(branch, bus, ref(pm, nw, :total_real_load), ref(pm, nw, :total_reactive_load))
...
"""
function constraint_mc_thermal_limit_to_damaged(pm::AbstractUnbalancedPowerModel, i::Int; nw::Int=nw_id_default)::Nothing
    branch = ref(pm, nw, :branch, i)
    t_idx = (i, branch["t_bus"], branch["f_bus"])
    bus = ref(pm, nw, :bus, branch["t_bus"])
...
"""
function constraint_mc_thermal_limit_from_ne(pm::AbstractUnbalancedPowerModel, i::Int; nw::Int=nw_id_default)::Nothing
    branch = ref(pm, nw, :branch_ne, i)
    f_idx = (i, branch["f_bus"], branch["t_bus"])
    bus = ref(pm, nw, :bus, branch["f_bus"])

    # using the same constraint store since a branch is either damaged or not damaged
    if !haskey(con(pm, nw), :mu_sm_branch_ne)
        con(pm, nw)[:mu_sm_branch_ne] = Dict{Tuple{Int,Int,Int},Vector{JuMP.ConstraintRef}}()
    end
...
"""
function constraint_mc_thermal_limit_to_ne(pm::AbstractUnbalancedPowerModel, i::Int; nw::Int=nw_id_default)::Nothing
    branch = ref(pm, nw, :branch_ne, i)
    t_idx = (i, branch["t_bus"], branch["f_bus"])
    bus = ref(pm, nw, :bus, branch["t_bus"])
...
"""
function constraint_mc_ampacity_from_damaged(pm::AbstractUnbalancedPowerModel, i::Int; nw::Int=nw_id_default)::Nothing
    branch = ref(pm, nw, :branch, i)
    f_idx = (i, branch["f_bus"], branch["t_bus"])
    bus = ref(pm, nw, :bus, branch["f_bus"])
    ...
    c_rating = calc_branch_current_max(branch, bus, ref(pm, nw, :total_real_load), ref(pm, nw, :total_reactive_load))
...
"""
function constraint_mc_ampacity_to_damaged(pm::AbstractUnbalancedPowerModel, i::Int; nw::Int=nw_id_default)::Nothing
    branch = ref(pm, nw, :branch, i)
    t_idx = (i, branch["t_bus"], branch["f_bus"])
    bus = ref(pm, nw, :bus, branch["t_bus"])
...
"""
function constraint_mc_ampacity_from_ne(pm::AbstractUnbalancedPowerModel, i::Int; nw::Int=nw_id_default)::Nothing
    branch = ref(pm, nw, :branch_ne, i)
    f_idx = (i, branch["f_bus"], branch["t_bus"])
    bus = ref(pm, nw, :bus, branch["f_bus"])

    if !haskey(con(pm, nw), :mu_cm_branch_ne)
        con(pm, nw)[:mu_cm_branch_ne] = Dict{Tuple{Int,Int,Int},Vector{JuMP.ConstraintRef}}()
    end
...
"""
function constraint_mc_ampacity_to_ne(pm::AbstractUnbalancedPowerModel, i::Int; nw::Int=nw_id_default)::Nothing
    branch = ref(pm, nw, :branch_ne, i)
    t_idx = (i, branch["t_bus"], branch["f_bus"])
    bus = ref(pm, nw, :bus, branch["t_bus"])
...
"""
function constraint_mc_voltage_angle_difference_damaged(pm::AbstractUnbalancedPowerModel, i::Int; nw::Int=nw_id_default)::Nothing
    branch = ref(pm, nw, :branch, i)
    f_bus = branch["f_bus"]
    t_bus = branch["t_bus"]
    f_idx = (i, f_bus, t_bus)
...
"""
function constraint_mc_voltage_angle_difference_ne(pm::AbstractUnbalancedPowerModel, i::Int; nw::Int=nw_id_default)::Nothing
    branch = ref(pm, nw, :branch_ne, i)
    f_bus = branch["f_bus"]
    t_bus = branch["t_bus"]
    f_idx = (i, f_bus, t_bus)
...
"""
function constraint_mc_ohms_yt_from_damaged(pm::AbstractUnbalancedPowerModel, i::Int; nw::Int=nw_id_default)
    branch = ref(pm, nw, :branch, i)
    f_bus = branch["f_bus"]
    t_bus = branch["t_bus"]
    f_idx = (i, f_bus, t_bus)
...
"""
function constraint_mc_ohms_yt_to_damaged(pm::AbstractUnbalancedPowerModel, i::Int; nw::Int=nw_id_default)
    branch = ref(pm, nw, :branch, i)
    f_bus = branch["f_bus"]
    t_bus = branch["t_bus"]
    f_idx = (i, f_bus, t_bus)
...
"""
function constraint_mc_ohms_yt_from_ne(pm::AbstractUnbalancedPowerModel, i::Int; nw::Int=nw_id_default)
    branch = ref(pm, nw, :branch_ne, i)
    f_bus = branch["f_bus"]
    t_bus = branch["t_bus"]
    f_idx = (i, f_bus, t_bus)
...
"""
function constraint_mc_ohms_yt_to_ne(pm::AbstractUnbalancedPowerModel, i::Int; nw::Int=nw_id_default)
    branch = ref(pm, nw, :branch_ne, i)
    f_bus = branch["f_bus"]
    t_bus = branch["t_bus"]
    f_idx = (i, f_bus, t_bus)
...
"""
function constraint_mc_gen_power_ne(pm::AbstractUnbalancedPowerModel, i::Int; nw::Int=nw_id_default)::Nothing
    gen = ref(pm, nw, :gen_ne, i)
...
"""
function constraint_radial_topology_ne(pm::AbstractUnbalancedPowerModel; nw::Int=nw_id_default, relax::Bool=false)
...
"""
function constraint_mc_switch_inline_ne_state_open_close(pm::AbstractUnbalancedPowerModel, i::Int; nw::Int=nw_id_default)
...
"""
function constraint_mc_switch_inline_ne_thermal_limit(pm::AbstractUnbalancedPowerModel, i::Int; nw::Int=nw_id_default)::Nothing
    switch = ref(pm, nw, :switch_inline_ne, i)
    f_idx = (i, switch["f_bus"], switch["t_bus"])

    if !haskey(con(pm, nw), :mu_sm_switch_inline_ne)
        con(pm, nw)[:mu_sm_switch_inline_ne] = Dict{Tuple{Int,Int,Int},Vector{JuMP.ConstraintRef}}()
    end
