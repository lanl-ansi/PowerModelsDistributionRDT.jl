...
function variable_he_s(pm::_PMD.AbstractUnbalancedWModels; nw::Int=nw_id_default, relax::Bool=false, report::Bool=true)
    if relax
     ...
     he_s_w_fr = var(pm, nw)[:he_s_w_fr] = Dict(i => JuMP.@variable(pm.model,
                 [t in terminals[branch[i]["f_bus"]]], base_name="$(nw)_he_s_w_fr_$(i)",
                 start=_PMD.comp_start_value(ref(pm, nw, :branch, i), "he_start", i, 0.0)
             ) for i in ref(pm, nw, :branch_harden)
         )

    he_s_w_to = var(pm, nw)[:he_s_w_to] = Dict(i => JuMP.@variable(pm.model,
...
function variable_xe_s(pm::_PMD.AbstractUnbalancedWModels; nw::Int=nw_id_default, relax::Bool=false, report::Bool=true)
    if relax
     ...
     terminals = Dict(i => bus["terminals"] for (i,bus) in ref(pm, nw, :bus))
     branch = ref(pm, nw, :branch_ne)

     xe_s_w_fr = var(pm, nw)[:xe_s_w_fr] = Dict(i => JuMP.@variable(pm.model,
                 [t in terminals[branch[i]["f_bus"]]], base_name="$(nw)_xe_s_w_fr_$(i)",
                 start=_PMD.comp_start_value(ref(pm, nw, :branch_ne, i), "xe_start", i, 0.0)
             ) for i in ids(pm, nw, :branch_ne)
         )

     xe_s_w_to = var(pm, nw)[:xe_s_w_to] = Dict(i => JuMP.@variable(pm.model,
...
function constraint_mc_power_balance_shed_ne(pm::_PMD.AbstractUnbalancedWModels, nw::Int, i::Int, terminals::Vector{Int}, grounded::Vector{Bool},
    ...
    p_ne     = get(var(pm, nw),   :p_ne, Dict()); _PMD._check_var_keys(p_ne, bus_arcs_ne, "active power", "branch_ne")
    q_ne     = get(var(pm, nw),   :q_ne, Dict()); _PMD._check_var_keys(q_ne, bus_arcs_ne, "reactive power", "branch_ne")
    ...
    pg_ne    = get(var(pm, nw),   :pg_ne, Dict()); _PMD._check_var_keys(pg_ne, bus_gens_ne, "active power", "generator_ne")
    qg_ne    = get(var(pm, nw),   :qg_ne, Dict()); _PMD._check_var_keys(qg_ne, bus_gens_ne, "reactive power", "generator_ne")
    ...
    psw_ne   = get(var(pm, nw),   :psw_inline_ne, Dict()); _PMD._check_var_keys(psw_ne, bus_arcs_sw_ne, "active power", "switch_inline_ne")
    qsw_ne   = get(var(pm, nw),   :qsw_inline_ne, Dict()); _PMD._check_var_keys(qsw_ne, bus_arcs_sw_ne, "reactive power", "switch_inline_ne")
    ...
    pt_ne    = get(var(pm, nw),   :pt_ne, Dict()); _PMD._check_var_keys(pt_ne, bus_arcs_trans_ne, "active power", "transformer_ne")
    qt_ne    = get(var(pm, nw),   :qt_ne, Dict()); _PMD._check_var_keys(qt_ne, bus_arcs_trans_ne, "reactive power", "transformer_ne")
...
function constraint_mc_ampacity_from_damaged(pm::_PMD.AbstractUnbalancedWModels, nw::Int, f_idx::Tuple{Int,Int,Int}, f_connections::Vector{Int}, c_rating::Vector{<:Real})::Nothing
    ...
    he_s_w_fr  = [var(pm, nw, :he_s_w_fr, f_idx[1])[c] for c in f_connections]

    for c in f_connections
       _IM.relaxation_product(pm.model, he_s, w_fr[c], he_s_w_fr[c])
    end

#    con(pm, nw, :mu_cm_branch)[f_idx] = mu_cm_fr = [JuMP.@constraint(pm.model, p_fr[idx]^2 + q_fr[idx]^2 .<= w_fr[idx] * c_rating[idx]^2 * he_s) for idx in findall(c_rating .< Inf)]
    con(pm, nw, :mu_cm_branch)[f_idx] = mu_cm_fr = [JuMP.@constraint(pm.model, p_fr[idx]^2 + q_fr[idx]^2 .<= he_s_w_fr[idx] * c_rating[idx]^2) for idx in f_connections]
...
function constraint_mc_ampacity_to_damaged(pm::_PMD.AbstractUnbalancedWModels, nw::Int, t_idx::Tuple{Int,Int,Int}, t_connections::Vector{Int}, c_rating::Vector{<:Real})::Nothing
    ...
    he_s_w_to  = [var(pm, nw, :he_s_w_to, t_idx[1])[c] for c in t_connections]

    for c in t_connections
       _IM.relaxation_product(pm.model, he_s, w_to[c], he_s_w_to[c])
    end

#    con(pm, nw, :mu_cm_branch)[t_idx] = mu_cm_to = [JuMP.@constraint(pm.model, p_to[idx]^2 + q_to[idx]^2 .<= w_to[idx] * c_rating[idx]^2 * he_s) for idx in findall(c_rating .< Inf)]
    con(pm, nw, :mu_cm_branch)[t_idx] = mu_cm_to = [JuMP.@constraint(pm.model, p_to[idx]^2 + q_to[idx]^2 .<= he_s_w_to[idx] * c_rating[idx]^2) for idx in t_connections]
...
function constraint_mc_ampacity_from_ne(pm::_PMD.AbstractUnbalancedWModels, nw::Int, f_idx::Tuple{Int,Int,Int}, f_connections::Vector{Int}, c_rating::Vector{<:Real})::Nothing
    ...
    xe_s_w_fr  = [var(pm, nw, :xe_s_w_fr, f_idx[1])[c] for c in f_connections]
...
#    con(pm, nw, :mu_cm_branch)[f_idx] = mu_cm_fr = [JuMP.@constraint(pm.model, p_fr[idx]^2 + q_fr[idx]^2 .<= w_fr[idx] * c_rating[idx]^2 * he_s) for idx in findall(c_rating .< Inf)]
    con(pm, nw, :mu_cm_branch_ne)[f_idx] = mu_cm_fr = [JuMP.@constraint(pm.model, p_fr[idx]^2 + q_fr[idx]^2 .<= xe_s_w_fr[idx] * c_rating[idx]^2) for idx in f_connections]
...
function constraint_mc_ampacity_to_ne(pm::_PMD.AbstractUnbalancedWModels, nw::Int, t_idx::Tuple{Int,Int,Int}, t_connections::Vector{Int}, c_rating::Vector{<:Real})::Nothing
    ...
    xe_s_w_to  = [var(pm, nw, :xe_s_w_to, t_idx[1])[c] for c in t_connections]
...
#    con(pm, nw, :mu_cm_branch)[t_idx] = mu_cm_to = [JuMP.@constraint(pm.model, p_to[idx]^2 + q_to[idx]^2 .<= w_to[idx] * c_rating[idx]^2 * he_s) for idx in findall(c_rating .< Inf)]
    con(pm, nw, :mu_cm_branch_ne)[t_idx] = mu_cm_to = [JuMP.@constraint(pm.model, p_to[idx]^2 + q_to[idx]^2 .<= xe_s_w_to[idx] * c_rating[idx]^2) for idx in t_connections]
...
function constraint_mc_switch_inline_ne_ampacity(pm::_PMD.AbstractUnbalancedWModels, nw::Int, f_idx::Tuple{Int,Int,Int}, f_connections::Vector{Int}, c_rating::Vector{<:Real})::Nothing
    ...
    con(pm, nw, :mu_cm_switch_inline_ne)[f_idx] = mu_cm_fr = [JuMP.@constraint(pm.model, psw_fr[idx]^2 + qsw_fr[idx]^2 .<= w_fr[idx] * c_rating[idx]^2) for idx in findall(c_rating .< Inf)]
