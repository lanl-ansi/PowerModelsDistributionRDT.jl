...
function constraint_mc_power_balance_shed_ne(pm::_PMD.AbstractUnbalancedDCPModel, nw::Int, i::Int, terminals::Vector{Int}, grounded::Vector{Bool}, bus_arcs::Vector{Tuple{Tuple{Int,Int,Int},Vector{Int}}},
    ...
    p = get(var(pm, nw), :p, Dict())
    _PMD._check_var_keys(p, bus_arcs, "active power", "branch")
    pg = get(var(pm, nw), :pg_bus, Dict())
    _PMD._check_var_keys(pg, bus_gens, "active power", "generator")
    ps = get(var(pm, nw), :ps, Dict())
    _PMD._check_var_keys(ps, bus_storage, "active power", "storage")
    psw = get(var(pm, nw), :psw, Dict())
    _PMD._check_var_keys(psw, bus_arcs_sw, "active power", "switch")
    pt = get(var(pm, nw), :pt, Dict())
    _PMD._check_var_keys(pt, bus_arcs_trans, "active power", "transformer")
    p_ne = get(var(pm, nw), :p_ne, Dict())
    _PMD._check_var_keys(p, bus_arcs_ne, "active power", "branch_ne")
    psw_ne = get(var(pm, nw), :psw_inline_ne, Dict())
    _PMD._check_var_keys(psw, bus_arcs_sw_ne, "active power", "switch_inline_ne")
    pt_ne = get(var(pm, nw), :pt_ne, Dict())
    _PMD._check_var_keys(pt, bus_arcs_trans_ne, "active power", "transformer_ne")
    pg_ne = get(var(pm, nw), :pg_ne, Dict())
    _PMD._check_var_keys(pg_ne, bus_gens_ne, "active power", "generator_ne")
